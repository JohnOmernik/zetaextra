...
RUN apt-get update && apt-get upgrade -y && apt-get install -y syslinux nano jq vim syslinux-utils pwgen openssh-server gcc pass libnss3 git curl && apt-get clean && apt-get autoremove -y && rm -rf /var/lib/apt/lists/*
